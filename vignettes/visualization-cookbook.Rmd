...
```{r load_packages}
# NOTE: You cannot load `tidyverse` because it will fail when the pkgdown site
# is built since tidyverse is not improted or suggested
library(tntpr)
library(ggplot2)
library(dplyr)
library(tidyr)
...
By default, `tntp_style()` uses the 'Segoe UI' font because this font is TNTP's base font. You must load the font into R. This process is beyond the scope of this tutorial, but the [`extrafont` package](https://github.com/wch/extrafont) can be used to load the font into R. Users can change the font with the font` parameter.
...
plt <- ggplot(ggplot2::mpg, aes(displ, hwy)) +
  geom_point() +
  labs(
    title = 'Cars with more displacement have\na lower MPG',
    subtitle = 'Relationship between displacement and MPG',
    x = 'MPG',
    y = 'Engine Displ.'
  )
...
1. Install the `extrafont` package with `install.packages('extrafont)`. Ensure you are using version 0.13 or higher.
2. Import fonts into the extrafont database with ``extrafont::font_import()`. This only needs to be done once.
3. Register the extra fonts with R using `extrafont::loadfonts()`. If you are using version 0.13 or higher you only need to do this once, right after you import the fonts in step 2. If you are using a version lower than 0.13 you will need to run this step in each new R session.
...
Note that in the multiple line chart, we overwrote `ggplot2`'s default color scheme to incorporate TNTP colors. We pulled the hex codes for the colors we wanted to use with `line_colors <- as.character(tntpr::colors_tntp[c('medium_blue', 'gold_3')])` and then added the custom colors with `scale_colour_manual(values = line_colors)`.
...
line_colors <- as.character(tntpr::colors_tntp[c('medium_blue', 'gold_3')])
```
...
ggplot(bar_df, aes(x = school_name, y = avg_salary)) +
  geom_bar(stat="identity", 
           position="identity", 
           fill= tntpr::colors_tntp[['medium_blue']]) +
  scale_y_continuous(labels = scales::dollar, limits = c(0, 5000)) +
  labs(
...
colors_to_use <- rev(as.character(colors_tntp[c('blue_1', 'blue_2', 'blue_4', 'blue_5')]))
```
...
colors_to_use <- colors_tntp[c('medium_blue', 'orange_3')] |> 
...
ggplot(scores_salary, aes(avg_salary, perc_rank_test_score)) +
  geom_point(color = colors_tntp['medium_blue'], alpha = 0.9, size = 3) +
  labs(
...
ggplot(dumbbell_df, aes(x = `under $2.5k`, xend = `$4.5k+`, y = fct_reorder(school_name, `under $2.5k`)), group = school_names) + 
  geom_dumbbell(
    colour = "#dddddd",
    size = 3,
    colour_x = colors_tntp['orange'],
    colour_xend = colors_tntp['medium_blue'],
    show.legend = TRUE
  ) +
  labs(title = "More teachers earn over $4.5k than under $2.5k",
...
ggplot(number_teachers_school, aes(n)) +
  geom_histogram(binwidth = 5, colour = "white", fill = "#00A4C7") +
  labs(
...
```{r}
ggplot(county_data, aes(x = t_salary, y = school_year, group = school_year)) +
...
```{r}
agree_scale_colors <- rev(c("#00A4C7", "#81D2EB", "#eeeeee", "#cccccc", "#aaaaaa", "#888888"))

# mapping of colors and responses for plot
agree_color_pal <- agree_scale_colors |> 
  set_names(agree_disagree_scale)

ggplot(teacher_survey_he_perc, aes(percent, question, fill = response)) +
  geom_col() +
  geom_text(
    aes(label = scales::percent(strong_response_percent, accuracy = 1), x = strong_response_percent - .04),
    color = 'white', fontface='bold', size = 5
  ) +
  scale_x_continuous(labels = scales::percent, limits = c(0,1)) +
  scale_fill_manual(values = agree_color_pal, drop = FALSE) +
...
```{r}
# colors to use
div_scale_colors <- tntpr::colors_tntp_likert[-4]
