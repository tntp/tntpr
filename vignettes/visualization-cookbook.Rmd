...
```{r}
tntp_colors("dark_green", "light_green")
```
...
A companion function is `show_tntp_colors()`, which will make a quick display of the given colors in the plot window. Again, you can use `show_tntp_colors()` with no arguments to display all available colors and names:
```{r}
show_tntp_colors()
```
...
ggplot(avg_salary, aes(x = school_year, y = avg_salary)) +
  geom_line(colour = tntp_colors("dark_green"), linewidth = 1) +
...
ggplot(line_plot_schools, aes(x = school_year, y = avg_salary, color = school_name)) +
  geom_line(size = 1) +
  scale_y_continuous(labels = scales::dollar, limits = c(0, 5000)) +
  scale_colour_manual(values = tntp_colors('dark_green', 'medium_blue')) +
  labs(
    title="Average Teacher Salaries",
    subtitle = "Teacher salaries remained constant between 2012 and 2015"
  ) +
...
ggplot(
  ...
  scale_fill_manual(values = tntp_colors('dark_green', 'medium_green', 'light_green', 'light_gray')) +
...
colors_to_use <- tntp_colors('medium_red', 'dark_green') |> 
...
ggplot(scores_salary, aes(avg_salary, perc_rank_test_score)) +
  geom_point(color = tntp_colors('dark_green'), alpha = 0.9, size = 3) +
  labs(
...
ggplot(dumbbell_df, aes(x = `under $2.5k`, xend = `$4.5k+`, y = fct_reorder(school_name, `under $2.5k`)), group = school_names) + 
  geom_dumbbell(
    colour = "#dddddd",
    size = 3,
    colour_x = tntp_colors('medium_red'),
...
ggplot(number_teachers_school, aes(n)) +
  geom_histogram(binwidth = 5, colour = "white", fill = tntp_colors('dark_green')) +
  labs(
...
ggplot(county_data, aes(x = t_salary, y = school_year, group = school_year, fill = factor(school_year))) +
  ...
  scale_fill_manual(values = tntp_palette('likert_4', reverse = TRUE))
```
...
```{r}
agree_scale_colors <- rev(c(tntp_colors('dark_green', 'medium_green'), "#eeeeee", "#cccccc", "#aaaaaa", "#888888"))
