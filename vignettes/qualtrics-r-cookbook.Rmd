...
vignette: >
  %\VignetteIndexEntry{TNTP Qualtrics/R Cookbook}
...
To get R to communicate with the Qualtrics API, you'll need to get an API Token from your Qualtrics account, and then securely store (and access) it from your script.
...
Instead we're going to save our token in our local operating system's credential manager and then have the script automatically pull it from there. We'll do this using the `tntp_cred()` function from `tntpr`, which is built around the [`keyring` package](https://keyring.r-lib.org/index.html).

Start by making a call to `tntp_cred()` at the top of your script and saving the value for authentication use later:
```{r, eval=FALSE}
q_key <- tntp_cred('QUALTRICS_TOKEN')
```

*Note: Using the same name for the credential across different scripts will make your life easier, so we recommend "QUALTRICS_TOKEN" for this credential, and the basic format "SYSTEM_TYPE" for other credentials.*

The first time you run your script, the function will notify you that it can't find the specified credential and ask if you want to save it now. Select "Yes", and then paste your API key into the prompt box to save it. Now that the credential is saved, calling `tntp_cred('QUALTRICS_TOKEN')` from any script on your computer will pull that value in automatically. If you need to update your token, use `tntp_cred_set('QUALTRICS_TOKEN')` to overwrite it.

Congrats! You've now securely saved your Qualtrics credentials, and you're ready to start pulling down survey data!

## Getting Data

### Set your Qualtrics authentication

The `qualtRics` package will temporarily store your credentials for use in all of its functions with the `qualtrics_api_credentials()` function. Call this at the top of your script (with your securely stored key) before using any of the other `qualtRics` functions:
```{r, eval = FALSE}
q_key <- tntp_cred('QUALTRICS_TOKEN')
...
### Survey ID
In addition to your authentication key, you'll need the survey ID for the survey you are trying to pull. This ID will be in the form "SV_**********", and the easiest way to get it is by navigating to the survey in your browser and copying it from the URL bar:
...
### Pulling Survey Data
Now that you're authenticated and have your survey id ready to go, it's time to pull a survey!
...
It's worth noting a few of the useful optional arguments in `fetch_survey()`:
...
Finally it's time to dig into the data! We'll cover a few different common analysis tasks here, including:

1.  Likert Matrix questions
1.  
