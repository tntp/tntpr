---
title: "Creating TNTP-style ggplot2 Graphs"
...
p_load(devtools, tidyverse)
if (!require("patchwork")) devtools::install_github("thomasp85/patchwork")
if (!require("tntpr")) devtools::install_github("tntpr/tntpr"); library(tntpr)
```
...
palettes <- c("default", "likert_4pt", "likert_5pt", "likert_6pt", "colors_tntp_classic")

palette_plots <- map(palettes, palette_tntp_scales) %>% 
  map2(palettes, plot_color_palette)

palette_plots[[1]] + (palette_plots[[2]] + palette_plots[[3]] + palette_plots[[4]]) + palette_plots[[5]] + plot_layout(ncol = 1)
...
```{r, fig.width=7, fig.length=5, echo=TRUE}
performance_data %>%
