...
#' # Text positions and colors
#' ggplot(mtcars, aes(mpg, wt)) +
#' geom_point() +
#'  labs(x        = "Fuel effiiency (mpg)",
#'       y        = "Weight (tons)",
#'       title    = "Main ggplot2 title",
#'       subtitle = "Subtitle for demo plot",
#'       caption  = "Brought to you by tntpr") +
#'  theme_tntp(base_size      = 12,
...
theme_tntp <- function(show_legend_title = TRUE,
  ...
  result <- theme_minimal(base_size   = base_size,
    ...
    theme(legend.position  = "bottom",        # Move legend to bottom
          ...
          plot.title       = element_text(hjust  = title_h_just,
                                          colour = title_color,
                                          size   = title_size),
          plot.subtitle    = element_text(hjust  = subtitle_h_just,
                                          colour = subtitle_color,
                                          size   = subtitle_size,
                                          margin = margin(7)),
          plot.caption     = element_text(hjust  = caption_h_just,
